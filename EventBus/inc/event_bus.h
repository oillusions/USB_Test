...
class EventBus {
    public:
        ...
        class Subscription {
            ...
            public:
                Subscription(EventBus* bus, std::string eventID, uint32_t handlerID): _bus(bus), _eventID(std::move(eventID)), _handlerID(handlerID) {};
        ...
        template<typename T>
        void publish(const std::string& eventID, T&& event) {
    ...
    private:
        ...
        struct HandlerRecord {
            ...
            HandlerRecord(std::string eventID, uint32_t handlerID, HandlerBase* handler): eventID(std::move(eventID)), handlerID(handlerID), handler(handler) {};
        };
